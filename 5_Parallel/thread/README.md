...
- 线程从启动例程返回，返回值就是线程的退出码(即线程函数中 return 返回值)。

- 线程可以被 **同一进程中的其他线程取消** (如果此线程是进程中的最后一个线程，则终止时进程也结束)。

- 线程调用 pthread_exit() 函数，如果此线程是进程中的最后一个线程，则终止时进程也结束，相比于用 return 返回，使用 pthread_exit() **可以实现线程栈的清理**。
